...
function SetorPanel() {
  ...
  const handleRetirar = (id: number, quantidade: number) => {
    setMateriais(prev =>
      prev.map(mat =>
        mat.id === id ? { ...mat, quantidade: mat.quantidade - quantidade } : mat
      )
    );
    setMensagens(prev => ({ ...prev, [id]: `${quantidade} unidade(s) retirada(s) com sucesso!` }));
    setBotaoAtivo(id);
    setTimeout(() => {
  ...
  const handleDevolver = (id: number, quantidade: number) => {
    setMateriais(prev =>
      prev.map(mat =>
        mat.id === id ? { ...mat, quantidade: mat.quantidade + quantidade } : mat
      )
    );
    setMensagens(prev => ({ ...prev, [id]: `${quantidade} unidade(s) devolvida(s) com sucesso!` }));
